> RETIRED — superseded by AV-DET-01. No per-type implementation exists; the single generic `src/components/avisos/AvisoDetail.tsx` serves ALL `AvisoType` values (`apps/web/src/lib/avisos.ts:9-18`). This doc is retained for catalog history only — do not implement against it. Recommendation-change renders through AV-DET-01; no `AvisoRecommendationChange.tsx` exists.
...
## Responsibility
Specialized TU RECOMENDACIÓN CAMBIÓ

## Details
Now Lukeville 14 min faster + Ver recomendación.
...
## File Reference
- Spec doc: `design/components/AV-REC-01-AvisoRecommendationChange.md` (this file)
