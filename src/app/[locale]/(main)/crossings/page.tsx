...
import { useCrossingsFilter, CrossingsFilterTabs, CrossingsFilterProvider } from "@/components/crossing/CrossingsFilterContext";

type CountryFilter = "all" | "MX" | "US";

function CrossingsPageInner() {
  ...
  useEffect(() => {
    if (userCountry && userCountry === "US" && filter === "MX") {
      setFilter("US");
    ...
    if (newExpandedId) {
      setTimeout(() => {
        ...
        if (cardEl) {
          ...
          const headerHeight = 120; // Header + search + filter tabs
          const margin = 16;
  ...
  if (showCountryPrompt) {
    return (
      <div className="min-h-dvh bg-background flex flex-col items-center justify-center px-5 py-32">
  ...
  return (
    <div>
      {/* Filter Pills - Below search, part of content */}
