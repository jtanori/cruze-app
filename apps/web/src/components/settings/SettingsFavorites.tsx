...
export function SettingsFavorites({ favorites, onSelect, onRemove }: SettingsFavoritesProps) {
  ...
  if (favorites.length === 0) {
    return <EmptyState title={t("favorites.empty")} description={t("favorites.emptyDescription")} />;
  }

  return (
    <div className="space-y-3">
      <h2 className="text-lg font-bold text-ink">{t("favorites.title")}</h2>
      <div className="space-y-2">
        {favorites.map((f) => (
          <div key={f.id} className="flex items-center gap-3 px-4 py-3 bg-surface border border-border rounded-[var(--radius-lg)]">
            ...
            {onRemove && <button onClick={() => onRemove(f.id)} className="text-xs text-muted hover:text-danger">{t("settings.favoritesPage.remove")}</button>}
          </div>
        ))}
      </div>
    </div>
